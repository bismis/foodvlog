...
def itm_details(request,c_slug,product_slug):
    try:
        prod=products.objects.get(category__slug=c_slug,slug=product_slug)
    except Exception as e:
        raise e
    return render(request,'single_item.html',{'pr':prod})
...
def searching(request):
    ...
    if 'q' in request.GET:
        ...
        prod=products.objects.all().filter(Q(name__contains=query)|Q(descr__contains=query))
